...
if(isset($_POST['createname'])){
      ...
      if(!empty($bucket)){
      $query ="SELECT accesskey,secretkey FROM accountinfo
              WHERE username='$name'";
     ...
     if($output=mysql_query($query))
     {
      while ($api= mysql_fetch_assoc($output)){
        ...
        $url="http://localhost:8080/new/main/".$accesskey."/".$secretkey."/".$bucket."/create";
...
if(isset($_POST['deletename'])){
     ...
     if($output=mysql_query($query))
     {
      while ($api= mysql_fetch_assoc($output)){
        $accesskey =$api['accesskey'];
        $secretkey =$api['secretkey'];
        $url="http://localhost:8080/new/main/".$accesskey."/".$secretkey."/".$delbucket."/delete";
        $response = file_get_contents($url);
        $response = new SimpleXMLElement($response);
        $output= $response->create;
     echo"<div class='modal fade' id='output' role='dialog'>
  ...
  if(isset($_POST['uploadform'])){
      ...
      if(!empty($upbucket)){
      $query ="SELECT accesskey,secretkey,endpoint FROM accountinfo
              WHERE username='$name'";
     ...
     if($output=mysql_query($query))
     {
      while ($api= mysql_fetch_assoc($output)){
        ...
        $url="http://localhost:8080/new/main/".$accesskey."/".$secretkey."/".$upbucket."/".
                 $filename."/eucalyptus/".$targetreplace."/".$endpoint;
        $response = file_get_contents($url);
        $response = new SimpleXMLElement($response);
        $output= $response->create;
     echo"<div class='modal fade' id='output' role='dialog'>
  ...
  if(isset($_POST['listform'])){
     ...
     if($output=mysql_query($query))
     {
      while ($api= mysql_fetch_assoc($output)){
        $accesskey =$api['accesskey'];
        $secretkey =$api['secretkey'];
        $url="http://localhost:8080/new/main/".$accesskey."/".$secretkey;
        $response = file_get_contents($url);
        $response = new SimpleXMLElement($response);
        $output= $response->create;
       echo"<div class='modal fade' id='output' role='dialog'>
...
<!--///// create container/////////////////////////////////////////////////////////////-->
    <div id="create" class="tab-pane fade in active">
      <h3>Create Container</h3>
       ...
       <form role="form" action="eucalyptus.php" method="POST"  >
...
<!--///// upload data////////////////////////////////////////////////////////////-->
    <div id="upload" class="tab-pane fade">
      <h3>Upload Data</h3>
        <form role="form" action="eucalyptus.php" method="POST" enctype="multipart/form-data" >
       <div class="form-group">
        <label for="usr">Container Name:</label>
